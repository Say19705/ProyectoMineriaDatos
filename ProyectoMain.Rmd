...
modelo <- train(y~., data = train, method = "rf", trControl = ct, na.action = na.exclude)
prediccionfvc <- predict(modelorf, newdata = test[,1:13])
test$predrfvc <- prediccionfvc
cfmCaret <- confusionMatrix(test$predrfvc,test$y)
cfmCaret
