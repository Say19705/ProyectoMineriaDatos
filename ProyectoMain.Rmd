...
#C:/Users/Ingebor Rubio/Desktop/Trabajos/Quinto Semestre/Mineria/Proyecto/ProyectoMineriaDatos/

knitr::opts_knit$set(root.dir="C:/Users/Ingebor Rubio/Desktop/Trabajos/Quinto Semestre/Mineria/Proyecto/ProyectoMineriaDatos/")
```
...
datos2013$AÑOOCU <- ifelse(datos2013$AÑOOCU == 'None', 2013, datos2013$AÑOOCU)
...
datos2014$AÑOOCU <- ifelse(datos2014$AÑOOCU == 'None', 2014, datos2014$AÑOOCU)
#-------------------Modificaciones 2015--------------------------
...
datosMatrimonio$GETHOM <- levels(datosMatrimonio$GETHOM)[datosMatrimonio$GETHOM]
datosMatrimonio$GETMUJ <- levels(datosMatrimonio$GETMUJ)[datosMatrimonio$GETMUJ]

datosMatrimonio$GETHOM <- ifelse(datosMatrimonio$GETHOM == "No Indigena", "No indigena",datosMatrimonio$GETHOM)
datosMatrimonio$GETMUJ <- ifelse(datosMatrimonio$GETMUJ == "No Indigena", "No indigena",datosMatrimonio$GETMUJ)

datosMatrimonio$GETHOM <- factor(datosMatrimonio$GETHOM)
datosMatrimonio$GETMUJ <- factor(datosMatrimonio$GETMUJ)
...
GETHOM: etnia del hombre
GETMUJ: etnia de la mujer
...
barplot(counts, las = 2, cex.names=.5 ,main = "Distribución de departamento de ocurrencia", xlab = "Departamento de ocurrencia")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Distribución de municipio de ocurrencia", xlab = "Municipio de ocurrencia")
```
...
barplot(counts, las = 1, cex.names=.8 ,main = "Distribución de escolaridad de los hombres", xlab = "Escolaridad del hombre")
```
...
barplot(counts, las = 1, cex.names=.8 ,main = "Distribución de escolaridad de las mujeres", xlab = "Escolaridad de las mujeres")
```
...
counts <- table(datosMatrimonio$GETHOM)
barplot(counts, las = 1, cex.names=1 ,main = "Distribución de etnia de los hombres", xlab = "Etnia del hombre")
```
...
counts <- table(datosMatrimonio$GETMUJ)
barplot(counts, las = 1, cex.names=1 ,main = "Distribución de etnia de las mujeres", xlab = "Etnia de las mujeres")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Departamentos con matrimonios con menores de edad", xlab = "Departamentos de ocurrencia")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Municipios con matrimonios con menores de edad", xlab = "Municipios de ocurrencia")
...
#departamentos
table(etniaMujer)
counts <- table(menorEdadGraph$GETMUJ)
barplot(counts, las = 1, cex.names=1 ,main = "Etnia de mujeres involucradas en matrimonios con menores de edad", xlab = "Etnia de las mujeres")
...
#departamentos
table(etniaHombre)
counts <- table(menorEdadGraph$GETHOM)
barplot(counts, las = 1, cex.names=1 ,main = "Etnia de hombres involucrados en matrimonios con menores de edad", xlab = "Etnia de los hombres")
...
## Ocurrencia de matrimonios con menores de edad desde 2009 a 2019
...
datosML$GETHOM <- as.factor(datosML$GETHOM)
datosML$GETMUJ <- as.factor(datosML$GETMUJ)
...
modelo.bp<-NB(datosML$y~., data=datosML,subset = trainRowsNumber, control=Weka_control(H=nnodos, N=100, G=TRUE), options=NULL)
...
Cómo último método de ML se utilizó Naive Bayes, que se basa en la fórmula estadística de bayes la cuál modela la probabilidad de que sucede un suceso en base a que otro previamente ha sucedido. Al ver el porcentaje de acierto del algoritmo, se puede notar que acertó en un 71% por lo que si parece haber patrones que determinan si la mujer es menor o mayor de edad. Adicionalmente, si vemos la matriz de confusión al momento de predecir si es mayor, suele ser bastante preciso, sin embargo, predijo que muchas mujeres eran mayores de edad cuando en realidad no lo eran, aún así, tambien es notable, al ver la matriz de confusión y el porcentaje de acierto que las variables si influencian el estado de la mujer al casarse.
De la misma manera, en cuanto a recursos computacionales el que más ha requerido han sido las redes neuronales, que llega a generar un modelo luego de transcurrir 30 minutos, mientras que con los otros dos métodos la cantidad de tiempo a esperar es mucho menor, por lo que también por estas razones es más conveniente usar Naive Bayes y árboles de decisión. En el caso de contar con mayor capacidad computacional podría intentar hacerse una red neuronal más completa con un tiempo menor y quizás obtener mejores resultados.

## Comparación de algoritmos 

Según los algoritmos con los que hemos trabajado, que han sido redes neuronales por RWEKA, árboles de decisión y naive bayes, hemos obtenido porcentajes de precisión satisfactorios, sin embargo, hay modelos que logran clasificar mejor que otros. En este caso el modelo que ha logrado una mejor precisión al predecir ha sido el de árboles de decisión con un porcentaje de 78%, seguido de Naive Bayes con un porcentaje de 71% y por último con RWEKA con un porcentaje de 66%.

## Conclusiones

Se ha logrado predecir si la edad de una mujer en un matrimonio supera la mayoría de edad o no la supera, es decir, si es mayor o menor de 18 años.

El alogritmo más efectivo para hacer la predicción ha sido los árboles de decisión por medio de rpart, con un porcentaje de acierto de 78%.

Fue posible observar que la cantidad de matrimonios con mujeres menores de edad (28,219) a lo largo de los años 2009 a 2019 representa casi la mitad de la cantidad de matrimonios con mujeres mayores de edad (54,183), lo cual no ha resultado ser tan grave como se esperaba pero sigue siendo una cantidad preocupante.


### Bibliografía

Chen, S., Webb, G., Liu, L. & Ma, Xin. (2019). A novel selective Naive Bayes algorithm. Elsevier. https://www.sciencedirect.com/science/article/abs/pii/S0950705119306185 

Du, J., Chen, Q., Peng, Y., Xiang, Y., Tao, C, Lu, Z. (2019). ML-Net: Multi-label classification of biomedical texts with deep neural networks. Journal of the american medical informatic Association. 1279-1285. https://doi.org/10.1093/jamia/ocz085

Freund, Y., Mason, L. (2000). The alternating decision tree learning algorithm.  http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.116.2945&rep=rep1&type=pdf 

Karim, M. Rahman, R. (2013).  Decision tree and Naive Bayes Algorithm for Classification and Generation of Actionable Knowledge for Direct Marketing. https://www.scirp.org/html/6-9301587_30463.htm?pagespeed=noscript 

Prensa Libre (2019). https://www.prensalibre.com/guatemala/comunitario/persisten-matrimonios-de-menores-de-edad-en-guatemala-pese-a-prohibicion/

unionestempranas. (s.f.) https://unionestempranas.org/edades-minimas-para-el-matrimonio-en-guatemala/