...
# ```{r minimos edades, echo=FALSE}
# res1 <- top_n(datosMatrimonio,-10,datosMatrimonio$EDADMUJ)
# print(res1$EDADMUJ)
# ```
Como se puede ver, existen muchos datos extraños dentro del data set, en donde la edad de los "cónyuges" es incluso menor a 5 años, sin embargo, revisando la lista de variables del INE de cada año, pudimos darnos cuenta que la edad más pequeña para mujeres durante los años fue 12 y de los hombres 14, por lo tanto se hizo un nuevo dataset para estudiar con precisión lo que deseamos estudiar. se filtro de la base de datos de matrimonios los valores en donde la edad de la mujer era menor a 18 (por el objetivo) y mayor a 12(para eliminar los datos no significativos o no lógicos), asimismo se filtró los datos en donde el hombre es mayor a 14 (de igual manera para eliminar esos datos confusos)
...
# ```{r minimos edades filtrado, echo=FALSE}
...
```
Filtrando los datos en donde las mujeres son menores a 18 años, es posible ber que una gran cantidad de mujeres se encuentran comprometidas con hombres mucho mayores a ellas, existen tados atípicos muy extensos y preocupantes, cabe mencionar que los datos que se encuentran en el fondo (muy pocos pero si existen) se encuentran así debido a que los datos no son indicados en el diagrama.
...
```
Muchos de estos se llevan a cabo en el departamento de Guatemala, donde una gran población son ladinos.
...
```
Se puede observar que no hay una diferencia significativa en estos grupos, esto si importante para saber si es cultural
...
```
Se puede observar como los datos se encuentran divididos en 4 grupos, verde, rojo, azul y negro. en donde todos los grupos, excepto en verde parecen estar bastante concentrados en un punto.
...
```
En este gráfico se puede observar como en cada combinación de gráficos sucede la combinación de grupos y se observan los clusters bastante separados.
...
En el primer gráfico, clusplot, s epuede ver detras las regiones y como se encuentran los datos, además se indica que se explica casi 63% del punto de variabilidad.
